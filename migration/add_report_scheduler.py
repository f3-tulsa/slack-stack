#!/usr/bin/env python3
"""Deprecated wrapper — use migration/paxminer_migrate.py --phase scheduler."""
...
def _rewrite_argv(argv: list[str]) -> list[str]:
    ...
    if "--env" not in new_argv:
        ...
        raise SystemExit(2)
    new_argv.extend(["--phase", "scheduler"])
    return new_argv


def main() -> int:
    print(
        "DEPRECATED: use migration/paxminer_migrate.py --phase scheduler ...",
        file=sys.stderr,
    )
