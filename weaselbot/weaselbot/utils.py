...
def _database_tls_enabled() -> bool:
    v = (os.getenv("DATABASE_TLS_ENABLED") or "").strip().lower()
...
def paxminer_schema_name() -> str:
    return os.environ.get("PAXMINER_SCHEMA", "paxminer")


def weaselbot_schema_name() -> str:
    return os.environ.get("WEASELBOT_SCHEMA", "weaselbot")


def home_region_date_tiers() -> tuple[int, int, int, int]:
    """Four lookback windows (days) for home-region resolution; from HOME_REGION_DATE_TIERS."""
...
def mysql_connection() -> Engine:
    ...
    Loads ``.env`` from the weaselbot package directory, then builds a SQLAlchemy engine
    with PyMySQL (TLS optional via ``DATABASE_TLS_ENABLED``).
    """

    os.chdir(os.path.dirname(os.path.abspath(__file__)))
    load_dotenv()
    user = quote_plus(os.environ["DATABASE_USER"])
    passwd = quote_plus(os.environ["DATABASE_PASSWORD"])
    host = os.environ["DATABASE_HOST"]
    port = os.environ.get("DATABASE_PORT", "3306")
    db_url = f"mysql+pymysql://{user}:{passwd}@{host}:{port}/?charset=utf8mb4"
    connect_args = {}
