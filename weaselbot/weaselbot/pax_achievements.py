...
def build_home_regions(schemas: pl.DataFrame, metadata: MetaData, engine: Engine) -> Selectable[Tuple[str, str, str]]:
    ...
    Iterates over each row in ``schemas`` and builds SQL queries that calculate user attendance
    for different lookback windows (30, 60, 90, 120 days).
    ...
    for row in schemas.iter_rows():
        ...
        try:
            ...
            s1, s2, s3, s4 = (
                home_region_sub_query(u, a, b, ao, dr) for dr in (d1, d2, d3, d4)
            )
...
def nation_sql(
    ...
    For each schema in ``schemas``, it joins the ``users``, ``bd_attendance``, ``beatdowns``, and ``aos``
    tables (YTD beatdowns). Queries are combined with ``union_all``.
    Raises:
...
def main():
    ...
    for row in schemas.iter_rows():
        ...
        with engine.begin() as cnxn:
            ...
            _tok = cnxn.execute(
                text(f"SELECT slack_token FROM `{wb}`.regions WHERE paxminer_schema = :s"),
                {"s": schema},
            ).scalar()
            token = decrypt_field(_tok) if _tok else None
            channel = cnxn.execute(
                text(f"SELECT achievement_channel FROM `{wb}`.regions WHERE paxminer_schema = :s"),
                {"s": schema},
            ).scalar()
        if channel is None:
