...
# Region (PAXminer) schema where achievements tables live — not the paxminer registry DB name.
def _require_region_schema() -> str:
    v = (os.environ.get("REGION_SCHEMA") or "").strip()
    if not v:
        raise RuntimeError(
            "REGION_SCHEMA must be set to your PAXminer per-region schema name "
            "(e.g. export REGION_SCHEMA=myregion). Used by this manual DDL script only."
        )
    return v


schema = _require_region_schema()
MYSQL_ENGINE = "InnoDB"
