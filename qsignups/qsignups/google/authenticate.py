...
def _blob_to_credentials(blob) -> Credentials | None:
    if blob is None:
        return None
    if isinstance(blob, str):
        plain = decrypt_field(blob)
        try:
    ...
    if isinstance(blob, dict):
        return Credentials.from_authorized_user_info(blob)
    return None


def __load_region_credentials(team_id):
    region: Region = DbManager.get_record(Region, team_id)
    if region and region.google_auth_data:
        return _blob_to_credentials(region.google_auth_data)
    return None
...
def __get_refreshed_credentials(team_id):
    ...
    if region_credentials and region_credentials.expired and region_credentials.refresh_token:
        ...
        DbManager.update_record(
            Region,
            team_id,
            {Region.google_auth_data: encrypt_field(region_credentials.to_json())},
        )
    return region_credentials
...
def connect(team_id):
    ...
    if region_credentials and region_credentials.valid:
        ...
        flow = InstalledAppFlow.from_client_config(client_config=GOOGLE_CREDENTIALS, scopes=SCOPES)
...
def disconnect(team_id):
    DbManager.update_record(Region, team_id, {Region.google_auth_data: None})
    return True
