...
def generate_manifest(outfolder, hostname):
  ...
  with open(output_manifest, "w", encoding="utf-8") as stream:
    stream.write(body.replace("__HOSTNAME__", hostname))
  return output_manifest

def generate_environment(outfolder, env, user_env_file, op_system):
  environment = load_environment(env, user_env_file)
  with open("manifest.json", "r", encoding="utf-8") as stream:
    manifest_obj = json.load(stream)

  environment["SLACK_SCOPES"] = ",".join(manifest_obj["oauth_config"]["scopes"]["bot"])
