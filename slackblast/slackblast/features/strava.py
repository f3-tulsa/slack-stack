...
def build_strava_form(body: dict, client: WebClient, logger: Logger, context: dict, region_record: Region):
    ...
    auth_blocks = [
        slack_orm.ImageBlock(
            image_url=static_image_url("btn_strava_connect_with_orange.png"),
...
def strava_exchange_token(event, context) -> dict:
    ...
    if user_records:
        ...
        DbManager.update_record(
            ...
            id=user_record.id,
            fields={
                User.strava_access_token: encrypt_field(response_json["access_token"]),
                User.strava_refresh_token: encrypt_field(response_json["refresh_token"]),
        ...
        DbManager.create_record(
            User(
                team_id=team_id,
                user_id=user_id,
                strava_access_token=encrypt_field(response_json["access_token"]),
                strava_refresh_token=encrypt_field(response_json["refresh_token"]),
...
def check_and_refresh_strava_token(user_record: User) -> str:
    ...
    if user_record.strava_expires_at < datetime.now():
        ...
        refresh_plain = decrypt_field(user_record.strava_refresh_token)
        res = requests.post(
            request_url,
            data={
        ...
        DbManager.update_record(
            ...
            id=user_record.id,
            fields={
                User.strava_access_token: encrypt_field(data["access_token"]),
                User.strava_refresh_token: encrypt_field(data["refresh_token"]),
                User.strava_expires_at: datetime.fromtimestamp(data["expires_at"]),
            },
        )
    else:
        access_token = decrypt_field(user_record.strava_access_token)
...
def handle_strava_modify(body: dict, client: WebClient, logger: Logger, context: dict, region_record: Region):
    ...
    blocks = [
        ...
        slack_orm.ImageBlock(
            image_url=static_image_url("api_logo_pwrdBy_strava_stack_light.png"),
