...
- Create Slackblast receipt tables: **`welcome_deliveries`** (Events API `team_join`) and **`interaction_claims`** (modal `view_submission` retries for backblast / preblast / Strava). See [ARCHITECTURE.md](ARCHITECTURE.md) (slackblast retry receipts).
...
For an existing environment that only needs new admin-schema objects, run `python migration/migrate_data.py --env test --bootstrap-only` (or `--env prod --bootstrap-only`). This applies idempotent bootstrap DDL, including `welcome_deliveries` and `interaction_claims`, without reading, dropping, or copying source regional tables. Run it **before** deploying Slackblast code that writes those tables; a missing `interaction_claims` table fails closed (backblast/preblast will not post).
