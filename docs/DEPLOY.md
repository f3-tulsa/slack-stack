...
3. Update **PAXMiner** Slack app from **`PAXminer/manifest-<stage>.json`** (**`SlackFunctionUrl`**, `reactions:write`, `emoji:read`, and the **Options Load URL** under *Interactivity & Shortcuts → Select Menus*, which must match the interactivity request URL). Reinstall so the `emoji:read` scope is granted. Without the scope the award-reaction picker falls back to a curated standard set; without the Options Load URL the picker cannot search at all.
