...
3. Update **PAXMiner** Slack app from **`PAXminer/manifest-<stage>.json`** (**`SlackFunctionUrl`**, `reactions:write`, `emoji:read`) and reinstall so `emoji:read` is granted. That scope is what lets the award-reaction field validate a name against the workspace's emoji on save; without it the field still saves but cannot verify the name exists. No **Options Load URL** is needed — the field is a text input, not an external select.
