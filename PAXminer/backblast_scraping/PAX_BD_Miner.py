...
ALLOWABLE_DAYS_BACKBLAST_DATE_VALID = int(os.environ.get("BACKBLAST_MAX_AGE_DAYS", "30"))
BACKBLAST_MAX_FUTURE_DAYS = int(os.environ.get("BACKBLAST_MAX_FUTURE_DAYS", "2"))
Q_ERROR_DM_HOUR = int(os.environ.get("Q_ERROR_DM_HOUR", "15"))
...
# Define AWS Database connection criteria (TLS + port from credentials.ini [aws])
...
def retrieve_date_line(backblast):
    ...
    if dateline:
        dateline = re.sub(r"xa0", " ", str(dateline), flags=re.I)
...
def isValidDate(date):
    ...
    forward_valid_date = (today + timedelta(days=BACKBLAST_MAX_FUTURE_DAYS)).strftime('%Y-%m-%d')
...
for index, row in f3_df.iterrows():
    ...
    msg_date = row['msg_date']
    text_tmp = row['text']
    text_tmp = re.sub(r"_\\xa0", " ", str(text_tmp))
    text_tmp = re.sub(r"\\xa0", " ", str(text_tmp))
    ...
    with mydb.cursor() as cursor:
        for index, row in bd_df.iterrows():
            ...
            if send_q_msg == 2:
                ...
                try:
                    if today.hour == Q_ERROR_DM_HOUR or  ( safe_cast(ts_edited, float) or safe_cast(timestamp, float) >= ( current_ts - 3600)):
                        send_q_msg = 2
                    else:
                        send_q_msg = 0
                except Exception as error:
